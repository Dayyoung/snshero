/**
 * StandardizedRewardGateway.ts
 * Unified SNS Point Settlement & Wallet Deposit Engine for all Mission Games
 * 
 * Rules:
 * - Normalized reward standard (~50 SNS Points / minute played)
 * - Minimum guaranteed deposit: 20P up to 260P (with skill, speed, and score multipliers)
 * - 100% atomic wallet deposit into LocalStorage (`hero_sns_history`)
 * - Real-time custom event dispatch (`hero_sns_updated`) for instantaneous UI updates
 */
 ...
 */
export function calculateAndDepositMissionReward(options: RewardCalculationOptions): RewardReceipt {
  const {
    gameId,
    gameTitle,
    durationSeconds = 60,
  ...
  // 6. Total Calculation (Guaranteed range: 20P ~ 260P)
  const rawTotal = Math.round((baseReward + scoreBonus + timeBonus + perfectBonus) * difficultyMult);
  const totalSns = isVictory ? Math.max(35, Math.min(260, rawTotal)) : Math.max(15, Math.min(80, Math.round(rawTotal * 0.4)));
