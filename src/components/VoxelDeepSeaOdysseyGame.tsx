  ...
  return (
    <div className="relative w-full h-[100dvh] bg-slate-950 text-white select-none overflow-hidden flex flex-col font-sans">
      ...
      <div className="absolute bottom-4 left-4 right-4 z-20 flex justify-between items-end pointer-events-none">
        ...
        {/* Depth & Torpedo Controls */}
        <div className="flex gap-2 pointer-events-auto">
          <button
            onPointerDown={() => (gameStateRef.current.keys.up = true)}
            onPointerUp={() => (gameStateRef.current.keys.up = false)}
            className="w-14 h-14 bg-sky-800/90 text-white rounded-2xl border border-sky-600 font-bold text-xs flex flex-col items-center justify-center cursor-pointer active:scale-95"
          >
            <span>상승 [Q]</span>
          </button>

          <button
            onPointerDown={() => (gameStateRef.current.keys.down = true)}
            onPointerUp={() => (gameStateRef.current.keys.down = false)}
            className="w-14 h-14 bg-sky-800/90 text-white rounded-2xl border border-sky-600 font-bold text-xs flex flex-col items-center justify-center cursor-pointer active:scale-95"
          >
            <span>하강 [E]</span>
          </button>
