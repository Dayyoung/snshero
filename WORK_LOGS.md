...
- **요청 사항**:
  ...
  - 카드 플레이 중 상단에 마진이 잡히지 않도록 수정 요청.
- **조치 내역**:
  - `KadanRpgView.tsx`에서 배틀 활성 여부(`isBattling = Boolean(battleEvent)`)를 정의.
  - 카드 플레이 중(`isBattling === true`)일 때:
    - 상단 `<header>` 요소를 조건부로 렌더링하지 않도록(`{!isBattling && <header ... />}`) 처리하여 `h-16`(64px) 및 상단 마진/보더를 완벽하게 제거.
    - `<main>` 컨테이너 역시 배틀 중일 때 `p-0 m-0 max-w-full pb-0 overflow-hidden`으로 전환하여 상단/하단 패딩에 의한 뷰포트 축소 차단.
    - 최상위 컨테이너를 `bg-[#060a14] overflow-hidden`으로 설정하여 배틀 화면이 상단 0(`top: 0`)부터 온전한 전체 높이로 표시되도록 구현.
- **검증 결과**:
  - `npm run lint` (`tsc --noEmit`): 0 오류 통과
  - `npm run build`: 프로덕션 빌드 성공 (`✓ built in 26.57s`)
- **Git 커밋/푸시**: 완료

---

## [2026-09-14 14:52 KST] [PC 화면 배틀 액션 버튼 영역 최소 높이 9개 카드판 높이(400px) 동기화 완료]
- **요청 사항**:
  - PC 화면에서 플로팅 배틀 액션 영역(QTE, 자동전투, 배속, 스킬 버튼)의 높이가 카드판보다 낮아서 9개 카드판이 가려 보이는 현상 개선.
  - 해당 영역의 최소 높이를 9개 카드판 높이로 설정 요청.
- **조치 내역**:
  - `PlayGameView.tsx`의 플로팅 액션 스택 컨테이너(`fixed inset-x-0 bottom-[130px] ... z-[160]`) 및 내부 버튼 스택(`flex flex-col items-end`)에:
    - 9개 카드판(3x3 보드) 높이에 정확히 맞춘 `lg:min-h-[400px]` (모바일/태블릿 `min-h-[300px] sm:min-h-[350px]`) 최소 높이 적용.
    - `items-center justify-center` 정렬을 적용하여 카드판과 수직 중앙 정렬을 완벽 동기화, 버튼 스택이 아래로 처져서 카드판 슬롯을 가리는 현상 원천 해결.
    - PC 화면 우측 여백 `lg:px-6` 확보로 카드판과의 안전 간격 유지.
- **검증 결과**:
  - `npm run lint` (`tsc --noEmit`): 0 오류 통과
  - `npm run build`: 프로덕션 빌드 성공 (`✓ built in 14.19s`)
- **Git 커밋/푸시**: 완료
