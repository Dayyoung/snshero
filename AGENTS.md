...
## 스프레드시트 개선 작업 진행 상태
- **마지막 수정 완료 항목 ID**: `[/report-ex] [모바일 13개 뷰포트 진입 차단 해제 검증 및 구글 폼 13건 일괄 보고 동기화 완료] (Row 32, 33, 34, 35, 36, 37, 44, 45, 46, 47, 48, 66, 93 완료)`
- **최종 업데이트 일시**: 2026-08-22 18:42 (KST)
...
## 금지 사항
- **`public/` 폴더 수정 절대 금지**: `public/` 폴더 내의 모든 정적 파일(이미지, PDF, 텍스트, 설정 파일, 소설 데이터 등)은 영구 보존 대상이며, 어떠한 경우에도 임의 수정, 생성, 덮어쓰기, 삭제를 절대 하지 않습니다.
