...
3. **코드 분석 및 구현**:
   ...
   - **화면 단순화 및 모달 허브화 절대 원칙 (Minimal First View)**: 새 기능이나 버튼 추가 시 첫 화면에 무분별하게 버튼을 평면 나열하지 않고, 상위/하위 메뉴 구조, 아코디언 메뉴, 또는 팝업(모달 허브) 형태로 깔끔하게 통합합니다.
   - 프로젝트 코딩 스타일에 맞춰 TypeScript, Tailwind CSS, i18n 번역(ko/en), `DESIGN.md`(Monospace, 웜크림/잉크 테마, 1px 보더) 및 `lowSpecMode` 처리 규칙을 준수합니다.
4. **검증 및 품질 확인**:
